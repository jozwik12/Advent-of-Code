...
'========================================================'
//[red, green, blue]
const POSSIBILITIES: number[] = [12,13,14]

//[red, green, blue]
type foo = [number, number, number]

const setIsPossible = (set: number[]): boolean => {
  return set[0] <= POSSIBILITIES[0] && set[1] <= POSSIBILITIES[1] && set[2] <= POSSIBILITIES[2]
}

let result1: number = 0

inputProper.split("\n")
  .map(game => game.split(new RegExp(";|:")))
  .map(splitGame => {
    splitGame.shift()
    return splitGame
  })
  .map(setString => setString
    .map(singleSet => {
      const returnArray: foo = [0, 0, 0]
      const splitSet = singleSet.split(",")
      splitSet.forEach(elem => {
            if (elem.includes("red")) returnArray[0] += parseInt(elem.replace(/\D/g, ""))
            if (elem.includes("green")) returnArray[1] += parseInt(elem.replace(/\D/g, ""))
            if (elem.includes("blue")) returnArray[2] += parseInt(elem.replace(/\D/g, ""))
      })
      return returnArray
    }))
  .forEach((gameArray, index) => {
    if (gameArray.every((set) => setIsPossible(set))) {
...
const result2 = inputProper.split("\n")
  .map(game => game.split(new RegExp(";|:")))
  .map(splitGame => {
    splitGame.shift()
    return splitGame
  ...
  .map(gameArray => {
    return gameArray.reduce((acc: foo, curr) => {
  ...
  .map(reducedArray => {
    return reducedArray.reduce((acc, val) => {
      return acc * val
    }, 1)
  })
  .reduce((acc, val) => {
