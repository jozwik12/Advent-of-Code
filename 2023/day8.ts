...
const inputOneExampleAlt: string = 
`LLR
...
const parsedInput: string[] = inputProper.split(/^\s*$/m)

const directions: string = parsedInput[0].trim().replace(/L/g, "0").replace(/R/g, "1")
...
parsedInput[1]
  ...
  .split("\n")
  .forEach(line => {
    const splitLine: string[] = line.split(/\W+/gm)
